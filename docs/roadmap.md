...
| v0.2 alpha — implemented | Web routes, HTML, JSON APIs, static export, SQLite CMS | HTTP integration tests, browser create/publish workflow, package tests |
| Next: types | Static types, definite assignment, typed schemas, source spans | Positive/negative language conformance suite |
| Future: scheduling | Effect-aware task graph, bounded concurrency, batching | Deterministic scheduling tests, cancellation and grant-propagation tests |
| Future: native | Native/Wasm runtime and compact bytecode format | Differential tests against this reference; speed, RSS and startup measurements |
