...
The hosted playground uses the reference VM in a Pyodide Web Worker. Source is passed as data to the SHIN compiler, not evaluated as Python. Model grants start disabled; the only available adapter echoes text. No external model connection, CMS database, user-source upload or persistent source storage is implemented. Output is rendered as text. The Python runtime is loaded from a pinned jsDelivr URL, so the browser, hosting and CDN dependency chain are trusted. VM budgets are complemented by a worker-termination timeout and Stop button; this does not imply a native SHIN backend or OS memory isolation. See [architecture and limits](docs/playground/README.md).
